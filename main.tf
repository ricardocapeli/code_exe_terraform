...
data "vsphere_resource_pool" "target_cluster" {
  name          = var.deploy_vsphere_pool
  datacenter_id = data.vsphere_datacenter.target_dc.id
}
...
resource "vsphere_virtual_machine" "kubernetes_master" {
  ...
  resource_pool_id = data.vsphere_resource_pool.target_cluster.id
  ...
  # provisioner "file" {
  #   source      = "cloud_init/cloud-init.yaml"
  #   destination = "/etc/cloud-init.yaml"
